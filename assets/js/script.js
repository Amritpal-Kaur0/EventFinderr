//  function to get the date range for the upcoming weekend
...
// Calculate travel time and distance between two points by Bing Maps API
...
// Calculate & display travel information for different travel mode
...
async function displayEvents(userLocation) {
  ...
  events.forEach((event) => {
    ...
    //  TO create an event title element and set its text content to the event name
    const eventTitle = document.createElement("div");
    eventTitle.classList.add("event-title");
    eventTitle.textContent = event.name;

    // to create event time element and set its text content to the event start time
    ...
    // to add event title and time elements to the event element
    eventElement.appendChild(eventTitle);
    eventElement.appendChild(eventTime);

    // to attach click event listener to the event element that shows travel information
    ...
    // To add event element to the event container
    eventContainer.appendChild(eventElement);
  });
}

// Filter events based on the user location and selected interests
// This function filters events based on user input for location, interests, and date
async function filterEvents() {
  // Get the user's location input
  const userLocation = document.getElementById("location-input").value;

  // Retrieve events for the user's location
  const events = await getEvents(userLocation);

  // To Clear the event container
  const eventContainer = document.getElementById("event-container");
  eventContainer.innerHTML = "";

  //  TO Check which interests the user has selected
  ...
  //  TO Get the user's date input
  const dateInput = document.getElementById("startDate").value;
  const inputDate = new Date(dateInput);

  //  TO Iterate through each event
  events.forEach((event) => {
    // Determine the category and family status of the event
    const category = event.classifications[0].segment.name.toLowerCase();
    const isFamily = event.classifications[0].family;

    //  TO Determine if the event should be shown based on the user's interests
    ...
    //  TO Determine if the event should be shown based on the user's date input
    ...
    //  TO Add the event to the event container if it should be shown
    if (
      showEventByDate &&
      (showSports || showMusic || showArtsTheater || showFamily)
    ) {
      //  To Create a new div element with the class "event"
      const eventElement = document.createElement("div");
      eventElement.classList.add("event");
      //  To Create a new div element with the class "event-title" and set its text content to the name of the event
      const eventTitle = document.createElement("div");
      eventTitle.classList.add("event-title");
      eventTitle.textContent = event.name;
      //  TO Create a new div element with the class "event-time" and set its text content to the start date/time of the event
      const eventTime = document.createElement("div");
      eventTime.classList.add("event-time");
      eventTime.textContent = new Date(
        event.dates.start.dateTime
      ).toLocaleString();
      //  TO Append the event title and event time elements to the event element
      eventElement.appendChild(eventTitle);
      eventElement.appendChild(eventTime);
      //  TO Add a click event listener to the event element
      eventElement.addEventListener("click", async function () {
        //  TO Get the element with the ID "selected-event" and clear its contents
        ...
        const clonedEventElement = eventElement.cloneNode(true);
        selectedEventContainer.appendChild(clonedEventElement);
        //  TO Calculate the travel information from the user's location to the event's venue and display it
        const destination =
          event._embedded.venues[0].address.line1 +
          ", " +
          event._embedded.venues[0].city.name;
...
// TO  Call filterEvents to initially show all events
filterEvents();
document.querySelectorAll('input[name="interest"]').forEach((checkbox) => {
  checkbox.addEventListener("change", function () {
    ...
    // TO  Check if all checkboxes are unchecked and display all events if they are
